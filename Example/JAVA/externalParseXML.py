...
class externalParseXML:
	...
	def vwriteTag(self,node):
		...
		varName=node.text
		varType=__serviceObj.getVarType(varName)
		if(varType=="int"):
			stri+="%d,"+varName+");"
		elif(varType=="Double"):
			stri+="%f,"+varName+");"
	...
	def swriteTag(self,node):
		self.__serviceObj.addSen("System.out.println(\""+node.text+"\");",self.tab)
	...
	def assignTag(self,node):
		...
		for child in node:
			if(child.tag=="input1"):
				input1 = self.callForNode(child[0])
			elif(child.tag=="input2"):
				input2 = self.callForNode(child[0])
			elif(child.tag=="output"):
				output = self.callForNode(child[0])
			else:
	...
	def startParsing(self):
		self.__serviceObj.addSen("import java.io.*;",self.tab)
		self.__serviceObj.addSen("",self.tab)
		self.__serviceObj.addSen("class A",self.tab)
		self.__serviceObj.addSen("{",self.tab)
		self.tab+=1
		self.__serviceObj.addSen("public static void main(String[] args)",self.tab)
		self.__serviceObj.addSen("{",self.tab)
		self.tab+=1
